...
public class Retire_Floor_Space_UnderBuilding extends BaseClass {
	
	String testCaseName="Retire_Floor_Space_UnderBuilding";
	...
	private static String testCategory = "Retire floor, space under building";
	...
	@Test(dataProvider="getData",description = "Retire floor, space under building")
	public void AddLandBuildingUnderBuilding(Hashtable<String,String> data,Method method) throws InterruptedException, IOException{
		...
		try {
			...
			testStatus=PortfolioLocationPg.RetireSpace(data.get("BuildingName"),data.get("FloorName"), data.get("SpaceName"));
			if(testStatus){
				BaseClass.reportTestCaseStatus(driver, logger, method.getName(), testStatus);			
			}else{
				BaseClass.reportTestCaseStatus(driver, logger, method.getName(), false);
			...
			testStatus=PortfolioLocationPg.RetireFloor(data.get("FloorName"), data.get("DispositionMethod"));
			if(testStatus){
				BaseClass.reportTestCaseStatus(driver, logger, method.getName(), testStatus);			
			}else{
				BaseClass.reportTestCaseStatus(driver, logger, method.getName(), false);
