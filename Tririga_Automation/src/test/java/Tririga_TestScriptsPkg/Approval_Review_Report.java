...
public class Approval_Review_Report extends BaseClass {
	
	String testCaseName="Approval_Review_Report";
	private WebDriver driver = null;
	private ExtentTest logger = null;
	
	public static LoginPage TririgaLoginCls=null;
	public static HomePage TririgaHomePage=null;
	public static ApprovalReviewsPage ApprovalReviewsPg=null;

	
	private static String testCategory = "Approval Review Report";
	...
	private void intialize() {
		...
		ApprovalReviewsPg=new ApprovalReviewsPage(driver,logger);
		
		logger.assignCategory(testCategory);
	}
	
	
	@Test(dataProvider="getData",description = "Approval Review Report")
	public void ApprovalReviewReport(Hashtable<String,String> data,Method method) throws InterruptedException, IOException{
		...
		try {
			...
			if(testStatus){
				...
				TririgaHomePage.NavigateToApprovalReviewsPg();			
			}else{
				BaseClass.reportTestCaseStatus(driver, logger, method.getName(), false);
			}
			
			testStatus=ApprovalReviewsPg.VerifyApprovalReviewReportColumnNames();
			if(testStatus){
				BaseClass.reportTestCaseStatus(driver, logger, method.getName(), testStatus);			
			}else{
				BaseClass.reportTestCaseStatus(driver, logger, method.getName(), false);
