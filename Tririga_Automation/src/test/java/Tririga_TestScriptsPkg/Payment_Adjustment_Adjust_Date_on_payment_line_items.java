...
import Tririga_PagesPkg.ProcessLeaseApprovalsPage;
...
public class Payment_Adjustment_Adjust_Date_on_payment_line_items extends BaseClass {

	String testCaseName = "Payment_Adjustment_Adjust_Date_on_payment_line_items";
	...
	private static final String testCategory = "Payment Adjustment - Adjust 'Date' on payment schedules  ";
	...
	public void DateTypePaymentAdjustment(Hashtable<String, String> data, Method method)
		...
		try {
			...
			if (testStatus) {
				...
				if (Status.trim().equalsIgnoreCase("Revision Accounting In Review")) {
					...
					TririgaHomePage.clickOnSingOut();
					testStatus = TririgaLoginCls.Tririga_Login(data.get("DCHA_Username"), data.get("DCHA_Password"));
					BaseClass.reportTestCaseStatus(driver, logger, method.getName() + addTimeStamp(), testStatus);
					...
					TririgaHomePage.NavigateToHomePg();
					TririgaHomePage.clickOnSingOut();
					TririgaLoginCls.Tririga_Login(data.get("Username"), data.get("Password"));
					parentWindow = WebBrowser.retrieveCurrentWindowHandle(driver, logger);
					TririgaHomePage.clickOnAllLeaseContractslnk();
