...
public class CreateNew_IncomeLeaseContract_ActivateContract extends BaseClass {
	
	String testCaseName="CreateNew_IncomeLeaseContract_ActivateContract";
	...
	private static String testCategory = "Create New Income Lease Contract and Activate Contract";
	...
	@Test(dataProvider="getData",description = "Create New Income Lease Contract and Activate Contract")
	public void CreateNewIncomeLeaseContractActivateContract(Hashtable<String,String> data,Method method) throws InterruptedException, IOException{
		...
		try {
			...
			testStatus=LeaseGeneralPg.EnterExpenseLease_ActiveContract_Details(testCaseName, data.get("LEASETYPE"), data.get("SECONDARYLEASETYPE"),
						data.get("CONTRACTSTATUS"),data.get("COMMENCEMENTDATE"),data.get("LEASEEXPIRATIONDATE"),data.get("BUSINESSUNIT"));
			if(testStatus){
				BaseClass.reportTestCaseStatus(driver, logger, method.getName(), testStatus);
			}else{
				BaseClass.reportTestCaseStatus(driver, logger, method.getName(), false);
			...
			testStatus=	LeaseContactsPg.SelectLookupData(data.get("TENANTORGANISATION"),data.get("LANDLORDORGANISATION"),data.get("REMITTO"));
			if(testStatus){
				BaseClass.reportTestCaseStatus(driver, logger, method.getName(), testStatus);
			}else{
				BaseClass.reportTestCaseStatus(driver, logger, method.getName(), false);
			...
			testStatus=LeasePaymentsPg.GeneratePaymentsSchedules(data.get("PAYMENTTYPE1"), data.get("INVOICETYPE"), data.get("FREQUENCY"), 
					data.get("PAYMENTDUEON"), data.get("FULLPAYMENTSTARTDATE"), data.get("NUMBEROFSCHEDULES"),data.get("MONTHSPERSCHEDULE"),
					data.get("TAXGROUP"), data.get("AMOUNTPERBASIS"),data.get("PAYMENTMETHOD"));
			if(testStatus){
				BaseClass.reportTestCaseStatus(driver, logger, method.getName(), testStatus);
			}else{
				BaseClass.reportTestCaseStatus(driver, logger, method.getName(), false);
